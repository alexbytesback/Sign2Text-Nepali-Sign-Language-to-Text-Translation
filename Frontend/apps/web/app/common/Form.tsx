...
const Form = ({
  ...
  return (
    <form id="contactForm" className={twMerge('', containerClass)}>
      {title && <h2 className={`${description ? 'mb-2' : 'mb-4'} text-2xl text-center font-dm-sans`}>{title}</h2>}
      ...
      {btn && (
        ...
        >
          <button type={btn.type || 'button'} className="btn bg-green-500 text-white sm:mb-0">
            {btn.title}
          </button>
        </div>
      )}
